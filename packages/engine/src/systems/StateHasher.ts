...
export class StateHasher {
   ...
   */
  static async hashState(state: GameState): Promise<string> {
   ...
   */
  private static canonicalize(state: GameState): string {
    const simplified = {
      ...
      players: state.players.map((player) => ({
        ...
        handIds: player.hand.map((c: Card) => c.id).sort(),
        deckIds: player.deck.map((c: Card) => c.id),
        graveyardIds: player.graveyard.map((c: Card) => c.id).sort(),
      })),
  ...
  private static async sha256(message: string): Promise<string> {
    ...
    if (typeof window !== 'undefined' && window.crypto && window.crypto.subtle) {
      ...
      const msgBuffer = new TextEncoder().encode(message);
      const hashBuffer = await (globalThis as any).crypto.subtle.digest('SHA-256', msgBuffer);
      const hashArray = Array.from(new Uint8Array(hashBuffer));
      return hashArray.map((b) => b.toString(16).padStart(2, '0')).join('');
    } else {
