...
describe('Level 1: Smoke Tests (Game Loop)', () => {
    ...
    it('should initialize the game correctly', async () => {
        ...
        expect(state.currentPlayer).toBe(0 as PlayerId); // Player 0 starts usually
    });
    ...
    it('should allow Player 0 to play a Unit', async () => {
        ...
        const unitCard = hand.find(c => c.type === 'unit' || (c as any).power !== undefined);
        ...
        const action = {
            ...
            cardId: unitCard!.id,
            targetSlot: { playerId: 0 as PlayerId, terrainId: 0 as any }
        };
        ...
        const state = engine.state;
        // Turn should pass
        expect(state.currentPlayer).toBe(1 as PlayerId);
        ...
        expect((slot.unit as any).id).toBe(unitCard!.id); 
        // Hand size reduced
    ...
    it('should end skirmish when both players pass', async () => {
        ...
        // Player 0 Passes
        await engine.processAction({ type: 'PASS', playerId: 0 as PlayerId });
        expect(engine.state.players[0].isDone).toBe(true);
        expect(engine.state.currentPlayer).toBe(1 as PlayerId);

        // Player 1 Passes
        await engine.processAction({ type: 'PASS', playerId: 1 as PlayerId });
