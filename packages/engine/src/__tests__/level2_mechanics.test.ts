...
import type { PlayerId, UnitCard, GameAction } from '../core/types';
...
describe('Level 2: Mechanics Tests (Combat & Rules)', () => {
    ...
    const findCard = (hand: any[], idPart: string) => hand.find(c => c.id.includes(idPart));
    ...
    it('should handle Combat, Damage, and Terrain Winning correctly', async () => {
        ...
        expect((t0_p0 as any).power).toBe(4); // Base power

        // Current Winner check (P0 4 vs P1 0) -> P0 waits? No, logic is instant usually for preview
        expect(engine.calculateTerrainWinner(0 as any)).toBe(0);
        ...
        await engine.processAction({
            ...
            cardId: archer.id,
            targetSlot: { playerId: 1 as PlayerId, terrainId: 0 as any }
        });
        ...
        expect((t0_p0_after as any).power).toBe(2);

        // Verify Winner Calculation
        // P0 (Veteran): 2 Power
        // P1 (Archer): 3 Power
        // Winner should be P1
        expect(engine.calculateTerrainWinner(0 as any)).toBe(1);
    }, 10000);
