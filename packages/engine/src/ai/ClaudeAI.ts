import type { GameAction, GameState, PlayerId, TerrainId, GameLogEntry } from '../core/types';
...
- Active: ${state.currentPlayer === this.playerId ? 'YOU' : 'OPPONENT'}
  ...
  private serializeCardCatalog(state: GameState): string {
    const seenCards = new Map<string, Card | any>();
    ...
    const cardDescriptions = Array.from(seenCards.values())
      .map(card => {
        ...
        const isUnit = (card as any).power !== undefined || ((card as any).getType && (card as any).getType() === 'unit');

        if (isUnit) {
          const power = (card as any).power !== undefined ? (card as any).power : (card as UnitCard).power;
