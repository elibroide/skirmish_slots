...
export class OngoingReactionTrait extends Trait {
    ...
    async applyReaction(event) {
        // Support both single effect (legacy) and multiple effects
        const effectConfigs = this.config.effects || [{
    ...
    async applyEffectConfig(effectConfig, event) {
        // Determine target(s)
        let targets = [];
    ...
    async applyEffect(target, event, effectConfig) {
        ...
        switch (effectConfig.effect) {
            ...
            case 'ADD_SLOT_MODIFIER':
                if (target.terrainId !== null) {
                    ...
                    slot.setModifier(slot.modifier + value);
                }
                break;
            case 'BOUNCE':
            ...
            case 'MOVE':
                ...
                if (target.terrainId !== null) {
                    ...
                    if (emptySlots.length > 0) {
                        ...
                        await target.move(newTerrainId);
                    }
                }
                break;
            case 'HEAL':
