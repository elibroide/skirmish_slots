...
export class ClaudeAI {
    constructor(playerId, config = {}) {
        ...
        Object.defineProperty(this, "totalInputTokens", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
        });
        Object.defineProperty(this, "totalOutputTokens", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
     ...
     */
    async selectAction(state, gameHistory) {
        try {
    ...
    buildPrompt(state, gameHistory) {
        ...
        if (gameHistory && gameHistory.length > 0) {
            ...
            sections.unshift('=== RECENT ACTIONS ===', this.serializeHistory(recentHistory), '');
        }
     ...
     */
    analyzeResources(state) {
        const myHand = state.players[this.playerId].hand.length;
        const oppHand = state.players[(1 - this.playerId)].hand.length;
        const diff = myHand - oppHand;
        let advice = "";
        if (diff > 0)
            advice = "You have CARD ADVANTAGE (+ cards). You can press the attack.";
        else if (diff < 0)
            advice = "You are BEHIND on cards. Consider passing early to recover CA.";
        else
            advice = "Cards are equal.";
...
Strategic Advice: ${advice}`;
     ...
     */
    analyzeBattlefield(state) {
        const myId = this.playerId;
        const oppId = (1 - this.playerId);
        let winning = 0;
        let losing = 0;
        let tied = 0;
        const terrainAnalysis = state.terrains.map((t, index) => {
            ...
            let status = "TIED";
            if (myTotal > oppTotal) {
                status = `WINNING (+${myTotal - oppTotal})`;
                winning++;
            }
            else if (myTotal < oppTotal) {
                status = `LOSING (${myTotal - oppTotal})`;
                losing++;
            }
            else {
                tied++;
...
${terrainAnalysis}`;
    ...
    serializeGameState(state) {
        const opponent = (1 - this.playerId);
...
- Active: ${state.currentPlayer === this.playerId ? 'YOU' : 'OPPONENT'}
    - You Passed: ${state.players[this.playerId].isDone ? 'Yes' : 'No'}
    - Opponent Passed: ${state.players[opponent].isDone ? 'Yes' : 'No'}`;
    }
    /**
     * Enhanced with Strategy Tags
     */
    serializeCardCatalog(state) {
        ...
        state.terrains.forEach(terrain => {
            [0, 1].forEach(pid => {
                const unit = terrain.slots[pid].unit;
                if (unit)
                    seenCards.set(unit.cardId, unit);
            });
        });
        if (seenCards.size === 0)
            return '=== CARD CATALOG ===\n(No cards visible)';
        const cardDescriptions = Array.from(seenCards.values())
            .map(card => {
            const tags = this.getStrategyTags(card.name);
            const tagString = tags ? ` ${tags}` : '';
            // Handle UnitState vs Card instance
            // UnitState objects don't have methods
            const isUnit = card.power !== undefined || (card.getType && card.getType() === 'unit');
            if (isUnit) {
                const power = card.power !== undefined ? card.power : card.power;
                return `- ${card.name} (${power} pwr): ${card.description}${tagString}`;
            }
            else {
...
${cardDescriptions}`;
    ...
    serializeHand(hand) {
        ...
        return hand.map(card => {
            const type = card.getType() === 'unit' ? `Unit, ${card.power} pwr` : 'Action';
            return `- ${card.name} (${type})`;
        }).join('\n');
    }
    serializeLegalActions(state) {
        if (!this.engine)
            return this.serializeLegalActionsFallback(state);
    ...
    serializeLegalActionsDetailed(state, actions) {
        return `=== LEGAL ACTIONS ===\n` + actions.map(action => {
            if (action.type === 'PASS')
    ...
    serializeLegalActionsFallback(state) {
    ...
    serializeHistory(history) {
        return history.map(entry => {
            ...
            if (entry.details.targetSlot)
                desc += ` at Terrain ${entry.details.targetSlot.terrainId}`;
